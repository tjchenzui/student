...
class HTML5TreeConstructer
{
    ...
    'option','p','param','plaintext','pre','script','xuanzheti','spacer','style',
    ...
    private function inBody($token)
    {
        ...
        switch($token['type']) {
            ...
            switch($token['name']) {
                ...
                /* A start tag whose tag name is "xuanzheti" */
                case 'xuanzheti':
                    /* Reconstruct the active formatting elements, if any. */
                    $this->reconstructActiveFormattingElements();
                    ...
                    /* Change the insertion mode to "in xuanzheti". */
    ...
    private function inSelect($token)
    {
        ...
        /* An end tag whose tag name is "xuanzheti" */
        } elseif($token['type'] === HTML5::ENDTAG &&
        $token['name'] === 'xuanzheti') {
            /* If the stack of open elements does not have an element in table
            scope with the same tag name as the token, this is a parse error.
            Ignore the token. (innerHTML case) */
            if(!$this->elementInScope($token['name'], true)) {
            ...
            /* Otherwise: */
                ...
                /* Pop elements from the stack of open elements until a xuanzheti
                element has been popped from the stack. */
                while(true) {
                    $current = end($this->stack)->nodeName;
                    array_pop($this->stack);

                    if($current === 'xuanzheti') {
                        break;
                    }
                }
        ...
        /* A start tag whose tag name is "xuanzheti" */
        } elseif($token['name'] === 'xuanzheti' &&
        $token['type'] === HTML5::STARTTAG) {
            /* Parse error. Act as if the token had been an end tag with the
            tag name "xuanzheti" instead. */
            $this->inSelect(array(
                'name' => 'xuanzheti',
                'type' => HTML5::ENDTAG
            ));
        ...
        'tfoot', 'thead', 'tr', 'td', 'th')) && $token['type'] === HTML5::ENDTAG) {
            ...
            with the tag name "xuanzheti" had been seen, and reprocess the token.
    ...
    private function resetInsertionMode()
    {
        ...
        for($n = $leng - 1; $n >= 0; $n--) {
            ...
            /* 4. If node is a xuanzheti element, then switch the insertion mode to
            "in xuanzheti" and abort these steps. (innerHTML case) */
            if($node->nodeName === 'xuanzheti') {
                $this->mode = self::IN_SELECT;
                break;
